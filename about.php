<!DOCTYPE html>
<html lang="en">
    <head>
        <?php include('head.php'); ?>
        <title>About Salt City Garage Doors, Garage Door Repair and Service Professionals</title>
    </head>
    <body>
    <?php include_once("analyticstracking.php") ?>
    <?php include('navbar.php'); ?>
        <div itemscope itemtype="http://schema.org/AboutPage">
            <meta itemprop="specialty" content="Garage Door Repair" />
            <span itemprop="about">
            <div class="container">

                <div class="row">
                    <div class="box">
                        <div class="col-lg-12">
                            <hr>
                            <h2 class="intro-text text-center">About
                                <strong>Salt City Garage Doors</strong>
                            </h2>
                            <hr>
                        </div>

                        <div class="local col-xs-12">
                            <img class="img-responsive" src="img/local-first-Utah.png" alt="Buy Local First, Local Utah Garage Door Service Company">
                        </div>
                        
                        <div class="col-xs-12 text-center">
                            <span itemprop="mainContentOfPage">
                                <p>We're a small, local company that strives to take the best possible care of our neighbors in Utah.</p>
                                <p>We strongly believe in same day service and will do our best to get you out of your bind. We also offer 24/7 Emergency services.</p>
                                <p>Our promise to you is to never sell what you don't need and to always have a smiling friendly technician knocking at your door doing quality honest work. Scroll down to see who we are and you'll always know who will be coming because we call every customer 15 minutes before we arrive.</p>
                            </span>
                        </div>
                        <div class="col-xs-12">
                            <img class="img-responsive" src="img/logo.png" alt="">
                        </div>
                        
                        <div class="clearfix"></div>
                    </div>
                </div>

                <div class="row">
                    <div class="box">
                        <div class="col-lg-12">
                            <hr>
                            <h2 class="intro-text text-center">Our
                                <strong>Team</strong>
                            </h2>
                            <hr>
                        </div>
                        <div class="col-ld-12 text-center">
                            <!-- <img class="img-responsive" src="http://placehold.it/750x450" alt="">
                            <h3>John Smith
                                <small>Job Title</small>
                            </h3> -->
                        </div>
                        <div class="col-lg-12 text-center">
                            <div class="picture">
                                <img class="img-responsive" src="img/Justen.jpg" alt="Justen Keesling">
                                    <h3>Justen Keesling
                                        <small>Owner/Technician</small>
                                    </h3>
                            </div>
                            <div class="box">
                                <div class="col-lg-12">
                                    <hr>
                                    <h2 class="intro-text text-center">Contact
                                        <strong></strong>
                                    </h2>
                                    <hr>
                                </div>
                        
                                <div class="col-md-12 text-center">
                                    <p>Phone:
                                        <strong><a href="tel:1-801-888-4359">801.888.4359</a></strong>
                                    </p>
                                    <p>Email:
                                        <strong><a href="mailto:josh@saltcitygaragedoors.com">justen@saltcitygaragedoors.com</a></strong>
                                    </p>
                                    <!-- <p>Address:
                                        <strong>595 N. Columbus St.
                                            <br>Salt Lake City, UT 84103</strong>
                                    </p> -->
                                </div>
                                <div class="clearfix"></div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            </span>

        <!-- /.container -->
        <?php include('footer.php'); ?>
        </div>
    </body>

</html>
